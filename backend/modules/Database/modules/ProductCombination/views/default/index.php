...
<div class="well product-combination-index">
    ...
    <?php echo GridView::widget([
        ...
        'columns' => [
            ...
            //     'format' => 'html',
            //     'value' => function ($model) {
            //         return "<span>{$model->stock}  <a href='#'>查看</a></span>";
            //     }
            ...
            [
                ...
                'expandIcon' => '<span class="fa fa-plus-square-o"></span>',
                'collapseIcon' => '<span class="fa fa-minus-square-o"></span>',
                'detailUrl' => Url::toRoute(['stock-detail']),
                'value' => function ($model, $key, $index, $column) {
                    return GridView::ROW_COLLAPSED;
                },
                'extraData' => function($model, $key, $index, $column){
                    return [
                        'combination_id' => $model->id,
                    ];
                },
                'contentOptions' => [
...
$js .= "jQuery(document).off('click', 'a.stock-detail').on('click', 'a.stock-detail', function(e) {
            e.preventDefault();
            jQuery('#content-edit').modal('show').find('.modal-content').html('" . Yii::t('app.c2', 'Loading...') . "').load(jQuery(e.currentTarget).attr('href'));
        });";

// $js .= "jQuery(document).off('click', 'a.expand-btn').on('click', 'a.expand-btn', function(e) {
//             e.preventDefault();
//              $(this).closest('tr')
//       .find('.kv-expand-icon')
//       .click();
