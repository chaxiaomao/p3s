...
{
    public $modelClass = 'backend\models\c2\entity\ProductCommitNote';
    ...
    public function actionEdit($id = null, $schedule_id = null)
    {
        ...
        $model->type = ProductionCommitNoteType::PRODUCT_COMMIT;
    ...
    {
        if (($model = \backend\models\c2\entity\ProductCommitNote::findOne($id)) !== null) {
            return $model;
        } else {
    ...
    {
        try {
            $model = \backend\models\c2\entity\ProductionSchedule::findOne($id);
            if ($model->state != ProductionScheduleState::FINISH) {
                if ($model) {
                    $model->allProductFinish();
                    $responseData = ResponseDatum::getSuccessDatum(['message' => Yii::t('cza', 'Operation completed successfully!')], $id);
                } else {
                    $responseData = ResponseDatum::getErrorDatum(['message' => Yii::t('cza', 'Error: operation can not finish!')], $id);
                }
